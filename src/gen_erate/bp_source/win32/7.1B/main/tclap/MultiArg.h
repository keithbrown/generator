 ...
 *  file:  MultiArg.h
 * 
 *  Copyright (c) 2003-2013 Mentor Graphics CorporationMentor Graphics CorporationMentor Graphics CorporationMentor Graphics Corporation, Michael E. Smoot .
 *  Copyright (c) 2004-2013 Mentor Graphics CorporationMentor Graphics CorporationMentor Graphics CorporationMentor Graphics Corporation, Michael E. Smoot, Daniel Aarno.
 *  All rights reverved.
 * 
...
/*========================================================================
 *
 * File:      $RCSfile: MultiArg.h,v $
 * Version:   $Revision: 1.8 $
 * Modified:  $Date: 2013/01/10 23:42:07 $
 *
...
class MultiArg : public Arg
{
	protected:
		...
		/**
		 * This list always contains the same number of elements as "_values".
		 * It holds the position of the argument on the command-line.
		 */
		 ...
		 */
		void _extractValue( const std::string& val, int position );
	...
	public:
		...
		/**
		 * Return the vector of command-line argument positions.
		 */
...
template<class T>
const std::vector<int>& MultiArg<T>::getPosition() { return _positions; }
   	...
   	if ( argMatches( flag ) )
   	{
		...
		if ( value == "" )
		{
			...
			if ( (unsigned int)*i < args.size() )
				_extractValue( args[*i], *i );
			else
				throw( ArgParseException("Missing a value for this argument!",
                                         toString() ) );
		}
		else 
			_extractValue( value, *i );
...
template<class T>
void MultiArg<T>::_extractValue( const std::string& val, int position ) 
{
