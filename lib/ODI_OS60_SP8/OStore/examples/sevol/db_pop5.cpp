...
int main(int, char **)
{
    ...
    printf("db_pop5: opening database %s\n", example_db5_name);

    os_database *db = os_database::create(example_db5_name);

    printf("db_pop5: beginning a transaction\n");

    OS_BEGIN_TXN(tx1, 0, os_transaction::update)

        printf("db_pop5: creating and populating eparts collection\n");
	...
	for (i = 0; i < 5; i++) {
	    ...
	    printf("db_pop5:     created epart with id %d and engineer %s\n",
		   pid, engineers[i]);
	    printf("             in os_database %s.\n", example_db5_name);
	}

        printf("db_pop5: creating eparts database root\n");

	os_typespec eparts_typespec("os_Collection<epart*>");
	db->create_root("eparts")->set_value(&eparts, &eparts_typespec);

        printf("db_pop5: creating and populating mparts collection\n");
	...
	for (i = 5; i < 10; i++) {
		...
		mpart(pid, new(db, employee::get_os_typespec())
		    employee(engineers[i]));
	    printf("db_pop5:     created mpart with id %d and engineer %s\n",
		   pid, engineers[i]);
	    printf("             in os_database %s.\n", example_db5_name);
	}
	os_typespec mparts_typespec("os_Collection<mpart*>");

        printf("db_pop5: creating mparts database root\n");

	db->create_root("mparts")->set_value(&mparts, &mparts_typespec);

        printf("db_pop5: ending transaction\n");

    OS_END_TXN(tx1)

    printf("db_pop5: closing database %s\n\n", example_db5_name);
