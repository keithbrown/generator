Example of a Simple Application Using ObjectStore
=================================================

This example and the example in the iostreams directory are
functionally similar:  they process requests for dinner
reservations at a restaurant.  However, they have different
implementations.  This example uses ObjectStore, and the
example in the iostreams directory uses iostreams.  Both
examples are intended to illustrate how to implement
ObjectStore by highlighting the differences from a
non-ObjectStore implementation.
...
an ObjectStore database and persistently stores one object, which
contains (among other things) information specifed on the command
line -- the name of the restaurant and the number of available
tables.  Each time the main application is invoked, this object is
read and (if a reservation is confirmed) updated.
