/* $Id: toolbar1.c,v 1.4 1997/10/14 19:59:31 bill Exp $
 *
 ...
 * Comments:
 *
 *   This sample demonstrates the basic usage of a toolbar.
 *   The toolbar contains a number of buttons, some of which 
 *   are in an exclusive group.  The exclusive group of buttons, and
 *   corresponding menu choices are kept in sync via attribute
 *   propagation.
 *
 *   The tooltip messages are posted automatically.  All that is needed is
 *   the call to vhelpStartupTooltip() and a resource 
 *   dictionary at the root level of the resource file called "Phrase".
 * 
 *   Similarly, the statusbar messages are also automatic.  All that 
 *   is required is the call to vhelpStartupStatus() and a resource
 *   dictionary at the root level of the resource file called "Status".
 *   And of course it is necessary for there to be a statusbar in the 
 *   dialog. 
 */
                                      ...
                                      vdict *attr2value)
{
    ...
    case ToolbarDialogSELECT_TOPS:
    {
        ...
        /* Sync up states of menu toggles & toolbar buttons */

        cmd_tag = (const vname*) vdictLoad(attr2value, vname_Tag);
        ...
        for (i = 0; i < (sizeof(_ToolbarDialogExclusives)/sizeof(char*)); i++)
        {
            ...
            vdictStore(changeDict, tag, 
                       (tag == cmd_tag) ? vname_On : vname_Off);
        }

        /*
         * Attributes offer a powerful way to change the state of objects
         * without concern about their actual class.  Since both menu items 
         * and buttons have an attribute named "Toggle", with this one
         * propagation we can set the values of all the toggle buttons and
         * toggle menu items at once.
         */
        vobjectPropagateChangeDict(vdialogGetObject(dialog), 
                                   vname_Toggle, changeDict);
