/* $Id: filechs2.c,v 1.8 1996/08/21 15:43:55 david Exp $
 *
 ...
 *       Extends the simple file chooser from demo #1 to include
 *       the ability to do filtering by file type. See the routine
 *	 OpenNotify for details.
 *
...
void OpenNotify (vmenuItem *item, vevent *event)
{
	...
	/* !!!
	 * Add our filters to the file chooser. We could use pattern
	 * matching filters but these filters aren't guaranteed to work
	 * with native choosers so we use system specific filters
	 * instead. See the vfilechsr routine vfilechsrMakeFilterSpecRoot-
	 * Resource for info on how to create these system filter 
	 * dictionaries.
	 * !!!
	 */

	filterResources = vresourceGet
	  (vapplicationGetResources(vapplicationGetCurrent()), 
	   TAG("Filter Types"));

	vfilechsrAddFilterBySystemType(fileChooser, 
	   filterResources, /* the root dictionary resource */
	   NULL,  /* pass NULL to install all the filters   */
	   NULL   /* pass NULL to use the "Default" item and the active */
	   );	  /*   filter */
    }
