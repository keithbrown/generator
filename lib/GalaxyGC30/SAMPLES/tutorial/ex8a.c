 ...
 * "Command Performance"
 *
 * This is a modification of tutorial8a.c to use function commands instead of
 * selector commands.
 *
 */
...
static int growFunc (vcommandFunction *command, vdict *context);
static int quitFunc (vcommandFunction *command, vdict *context);
static int shrinkFunc (vcommandFunction *command, vdict *context);
static vdict *createCommandDict (struct functionInitializer *functions);
static vdict *createKeyDict (struct functionInitializer *functions);

static struct functionInitializer commands [] = {
  { "grow",        growFunc,   vCOMMAND('g')  },
  { "quit",        quitFunc,   vCOMMAND('q')  },
  { "shrink",      shrinkFunc, vCOMMAND('s')  },
  { NULL }
};
...

{
  ...
  /* Use a helper function to create our command dictionary */
  commandDict = createCommandDict (commands);
  ...
  keyDict = createKeyDict (commands);
...
vdict *createCommandDict (struct functionInitializer *commands)

{

  struct functionInitializer  *scan;
  vcommandFunction            *function;
  vdict                       *commandDict = vdictCreate (vnameHash);

  for (scan = commands; scan->tag != NULL; scan++) {
    function = vcommandCreateFunction ();
    vcommandSetFunctionIssue (function, scan->func);
    vdictStore (commandDict, 
		vnameInternGlobalLiteral (scan->tag), 
		function);
  }
...
vdict *createKeyDict (struct functionInitializer *keys)
