 ...
 * ex7a.c
 *
 * "The Quit Button casts a shadow"
 *
 */
...

{
  ...
  foregroundColor = vdialogDetermineItemColor(item, vdrawCOLOR_FOREGROUND);
  shadowColor = vdialogDetermineItemColor(item, vdrawCOLOR_SHADOW_BOTTOM);
  ...
  vdrawSetColor (shadowColor);
  vdrawMoveTo (rect2.x + 1, rect2.y - 1);
  vdrawAShow (0, 0, vbuttonGetTitle (button));

  vdrawSetColor (foregroundColor);
  vdrawMoveTo (rect2.x - 1, rect2.y + 1);
  vdrawAShow (0, 0, vbuttonGetTitle (button));
