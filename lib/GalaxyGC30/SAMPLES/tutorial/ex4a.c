 ...
 * "The Three Buttons"
 *
 * This version won't let the shrink button shrink us
 * into oblivion.
 *
 */
...

{
  ...
  /*
   *  This is the brute force way of forcing the dialog not to get
   *  too small.
   *
   */

  if (rect.w < DIALOG_MIN_SIZE)
    rect.w = DIALOG_MIN_SIZE;

  if (rect.h < DIALOG_MIN_SIZE)
    rect.h = DIALOG_MIN_SIZE;
